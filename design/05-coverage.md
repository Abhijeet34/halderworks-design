...
| A high-contrast theme | covered | `75-spec-sheet.md#high-contrast-which-answers-prefers-contrast-more-and-nothing-else` | two media queries, two answers. `prefers-contrast: more` is a per-role re-solve that `tools/build.py` emits into `tokens/tokens.css` from the seed with every 4.5:1 floor raised to 7:1 and every 3:1 floor to 4.5:1, and `tools/contrast.py` certifies it independently: 108 text pairs and 90 non-text pairs, 0 below either, with a 0.06 minimum step between the text roles. `forced-colors: active` is the filled level's transparent border, [50-surface-texture.md](50-surface-texture.md#under-forced-colours). `exports/` carries the default themes only |
