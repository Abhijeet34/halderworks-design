...
| **Purple-to-blue gradient hero on white** | the default hero of every AI product page since 2023 | no gradients. There is no gradient token and there is no plan to add one. 14 of 44 measured sites paint a gradient on a large element; none of the ones this system took principles from paints one behind text |
...
| **An illustration per empty state** | it makes an empty screen feel designed | no illustration set exists here and inventing one per empty state is how a product ends up with six unrelated drawings |
